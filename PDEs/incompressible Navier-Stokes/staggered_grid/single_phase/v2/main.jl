...
struct Parameters{T<:AbstractFloat}
    ...
    function Parameters(; Lx::T, Ly::T, dynamic_viscosity::T, density::T, horizontal_velocity::T, nx, ny, ep::T, p::T, is_solid::BitMatrix) where T<:AbstractFloat
        dx = Lx / nx
        dy = Ly / ny
...
function get_solution(saved_values, t, param)
    ...
    return vx_center, vy_center, pressure, vx, vy
end
...
solver = Tsit5((stage_limiter!)=apply_boundary_and_obstacle_conditions_limiter!)
tol = type(1e-4)
...
vx_real = @lift get_solution(saved_values, $t, param)[4]

vy_real = @lift get_solution(saved_values, $t, param)[5]
