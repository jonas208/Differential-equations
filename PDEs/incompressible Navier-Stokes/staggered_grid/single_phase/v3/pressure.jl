...
function get_coefficient_mat(is_solid::BitMatrix, nx_pressure, ny_pressure, dx::T, dy::T, density::T) where T
    rowPtr = Int[]
    colVal = Int[]
    nzVal = T[]
    ...
    for j in 1:ny_pressure, i in 1:nx_pressure
        ...
        # Hindernisse
        if is_solid[i, j]
            push!(colVal, linear_indices(i, j, nx_pressure))
            push!(nzVal, T(1.0))
        ...
        if j > 1 && !is_solid[i, j-1] # kein undurchlässiger Rand
        ...
        # (j - 1) * nx + (i-1) = row - 1
        if i > 1 && !is_solid[i-1, j] # kein Einströmungsbereich und kein undurchlässiger Rand
            inv_density_face = T(1.0) / density
            ...
            row_length += 1
        end # Einströmungsbereich oder undurchlässiger Rand, d.h. kein Fluss wegen Einströmungsbedingung oder No-Slip-Bedingung (Neumann)
        ...
        # (j - 1) * nx + (i+1) = row + 1
        inv_density_face = T(1.0) / density
        coeff = inv_density_face * T(1.0) / dx^2

        if i < nx_pressure # kein Ausströmungsbereich
            if !is_solid[i+1, j] # kein undurchlässiger Rand
                push!(colVal, linear_indices(i + 1, j, nx_pressure))
                push!(nzVal, coeff)
        ...
        else # Ausströmungsbereich, d.h. kein Eintrag wegen Ausströmungsbedingung (Dirichlet)
            # Fluss findet statt
            nzVal[diag_index] -= coeff
        end
        ...
        if j < ny_pressure && !is_solid[i, j+1] # kein undurchlässiger Rand
            inv_density_face = T(1.0) / density
            coeff = inv_density_face / dy^2
...
function calculate_divergence_kernel!(divergence::CuDeviceMatrix{T}, is_solid::CuDeviceMatrix{Bool}, vx::CuDeviceMatrix{T}, vy::CuDeviceMatrix{T}, time_step::T, dx::T, dy::T, nx_pressure, ny_pressure) where T
...
function calculate_divergence!(vx::CuMatrix{T}, vy::CuMatrix{T}, time_step::T, param) where T
    kernel = @cuda launch = false calculate_divergence_kernel!(param.divergence, param.is_solid, vx, vy, time_step, param.dx, param.dy, param.nx_pressure, param.ny_pressure)
    ...
    kernel(param.divergence, param.is_solid, vx, vy, time_step, param.dx, param.dy, param.nx_pressure, param.ny_pressure, threads=(x_threads, y_threads), blocks=(x_blocks, y_blocks))
end
