...
# fps = 75 # Bildwiederholrate der Animationen

# Zeitintervall
tspan = (type(0.0), type(20.0))
...
coefficient_mat_cpu = get_coefficient_mat_cpu(divergence_mat_cpu, nx, ny, dx, dy, is_solid_cpu)
...
# Parameter
fluxes_x_cpu = Array{type, 3}(undef, 3, nx-3, ny-3)
fluxes_y_cpu = Array{type, 3}(undef, 3, nx-3, ny-3)
param_cpu = (Lx, Ly, kinematic_viscosity, density, horizontal_velocity, simulate_smoke, is_solid_cpu, nx, ny, dx, dy, coefficient_mat_cpu, pressure_vec_cpu, divergence_mat_cpu, lin_solve_cpu, use_weno, ep, p, fluxes_x_cpu, fluxes_y_cpu)
...
c0s = [c0(x, y) for x in xs, y in ys]

apply_boundary_conditions!(vx0s, vy0s, c0s, horizontal_velocity, is_solid_cpu)

u0_cpu = vcat(vec(vx0s), vec(vy0s), vec(c0s))
